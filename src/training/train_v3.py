...
LR         = 0.001
PATIENCE   = 12
SAVE_PATH  = f'models/model_{BITS}bit_v3.pt'
DEVICE     = torch.device('cpu')

# ── Deeper model (6 layers) ───────────────────────────────────────
def build_deep_model(bits=8):
...
# ── Model ─────────────────────────────────────────────────────────
print(f"\nBuilding deep {BITS}-bit model (6 layers)...")
model     = build_deep_model(BITS).to(DEVICE)
...
# ── Comparison summary ────────────────────────────────────────────
print("\nModel comparison:")
print(f"  Original 8-bit (4 layers) : 81.97%")
print(f"  v2 SMOTETomek (4 layers)  : 81.71%")
print(f"  v3 Deep+SMOTE (6 layers)  : {best_acc*100:.2f}%")